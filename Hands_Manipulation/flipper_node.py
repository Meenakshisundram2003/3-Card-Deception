...
class FlipperActionServer(Node):
    ...
    def execute_flip_sequence(self, goal_handle):
        ...
        try:
            ...
            system_instruction = (
                "You are 'Stretch', a robotic magician. You are performing the Three Card Monte. "
                "Your personality is witty, confident, and slightly dramatic. "
                "CRITICAL: Keep all responses short (max 20 words) so you don't bore the audience."
            )
            
            messages = [{"role": "system", "content": system_instruction}]
            
            # 2. STEP 1: Greet
            messages.append({"role": "user", "content": "Greet the guest and ask for the name."})
            
            response_text = llm_response(messages) # "Greetings! I am Stretch. Who is brave enough to play? What is your name?"
            speak(response_text)
            ...
            response_text = llm_response(messages) # "Welcome, Sundar. Prepare to be amazed."
            speak(response_text)
            ...
            instruction_prompt = (
                "Explain the rules: They must pick a card, look at it, put it back, "
                "and then swap the positions of the *other two* cards. Tell them to do it now."
            )
            messages.append({"role": "user", "content": instruction_prompt})
            
            response_text = llm_response(messages) # "Pick a card, memorize it, and replace it. Then, swap the other two cards to confuse me!"
            speak(response_text)
